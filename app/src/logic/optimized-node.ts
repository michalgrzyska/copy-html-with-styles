...
export class OptimizedNode {
    ...
    constructor(
        ...
        this.children = this.getChildren(serializedNode);
    ...
    private getChildren(node: SerializedNode): OptimizedNode[] {
        ...
        return children.map((x) => new OptimizedNode(x, this.optionsForm, false));
    }
    ...
    private getAttributesFiltered(attributes?: Record<string, any>): Record<string, any> {
        ...
        if (!this.optionsForm.includeOthers) {
            Object.keys(attributes).forEach((x) => {
                if (!BASE_ATTRIBS.includes(x)) {
                    delete attributes[x];
                }
            });
        }
